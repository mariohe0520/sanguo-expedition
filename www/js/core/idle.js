...
// Numeric constants loaded from GAME_BALANCE.IDLE
const Idle = {
  get RATES() {
    const _I = (typeof GAME_BALANCE !== 'undefined') ? GAME_BALANCE.IDLE : {};
    return {
      goldPerMin: _I.BASE_GOLD_PER_MIN || 2,
      expPerMin: _I.BASE_EXP_PER_MIN || 1,
      lootChancePerHour: _I.LOOT_CHANCE_PER_HOUR || 0.1,
    };
  },
  get MAX_HOURS() {
    return (typeof GAME_BALANCE !== 'undefined' && GAME_BALANCE.IDLE) ? GAME_BALANCE.IDLE.MAX_HOURS : 24;
  },

  /**
   * 计算并发放离线收益
   * @returns {{ gold: number, exp: number, minutes: number, loot: Array, shardDrops: Array, chapterMult: number, message: string }|null} 离线收益详情，时间不足则返回 null
   */
  collectRewards() {
    const state = Storage.getIdleState();
    const now = Date.now();

    // Anti-cheat: detect time reversal (clock set backwards)
    if (now < state.lastCollect) {
      console.warn('[Idle] Time reversal detected. now=' + now + ' lastCollect=' + state.lastCollect);
      // Update lastCollect to now so future collections work normally, but give 0 reward
      state.lastCollect = now;
      Storage.saveIdleState(state);
      return null;
    }

    let elapsed = (now - state.lastCollect) / 60000; // minutes

    // Anti-cheat: if elapsed > 48 hours, cap to 24 hours and log
    if (elapsed > 48 * 60) {
      console.warn('[Idle] Suspicious time gap: ' + Math.floor(elapsed / 60) + 'h. Capping to 24h.');
      elapsed = this.MAX_HOURS * 60;
    }

    // Normal cap at MAX_HOURS (24h)
    elapsed = Math.min(elapsed, this.MAX_HOURS * 60);
    ...
    const _I = (typeof GAME_BALANCE !== 'undefined') ? GAME_BALANCE.IDLE : {};
    const levelMult = 1 + (player.level - 1) * (_I.LEVEL_MULT_PER_LEVEL || 0.15);
    ...
    const chapterMult = 1 + chapter * (_I.CHAPTER_MULT_PER_CHAPTER || 0.5); // Ch1 = 1.5x, Ch5 = 3.5x, Ch10 = 6x
  ...
  /**
   * 预览当前待领取的离线收益（不修改存档）
   * @returns {{ minutes: number, gold: number, exp: number, chapterMult: number, expeditionCount: number }|null}
   */
  getRewardPreview() {
    const state = Storage.getIdleState();
    const now = Date.now();
    if (now < state.lastCollect) return null; // time reversal
    let elapsed = Math.min((now - state.lastCollect) / 60000, this.MAX_HOURS * 60);
    if (elapsed > 48 * 60) elapsed = this.MAX_HOURS * 60; // cap suspicious gaps
    if (elapsed < 1) return null;

    const player = Storage.getPlayer();
    const _I2 = (typeof GAME_BALANCE !== 'undefined') ? GAME_BALANCE.IDLE : {};
    const levelMult = 1 + (player.level - 1) * (_I2.LEVEL_MULT_PER_LEVEL || 0.15);
    const progress = Storage.getCampaignProgress();
    const chapter = progress.chapter || 1;
    const chapterMult = 1 + chapter * (_I2.CHAPTER_MULT_PER_CHAPTER || 0.5);
    const expeditionBonus = this._getExpeditionBonus();

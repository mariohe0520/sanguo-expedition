// 三国·天命 — Storage (core/storage.js)
// All localStorage keys use prefix 'sg-' to avoid collisions.
const Storage = {
  /**
   * 从 localStorage 读取数据
   * @param {string} k - 存储键名（自动加 sg- 前缀）
   * @param {*} def - 默认值（读取失败时返回）
   * @returns {*} 解析后的数据或默认值
   */
   ...
   * 向 localStorage 写入数据
   * @param {string} k - 存储键名（自动加 sg- 前缀）
   * @param {*} v - 要存储的值（自动 JSON.stringify）
   */
  ...
  /**
   * 增减金币并持久化
   * @param {number} n - 增加的金币数（负数为扣除）
   * @returns {number} 操作后的金币总额
   */
  ...
  getHeroEffectiveStats(heroId) {
    ...
    const mult = (1 + (level - 1) * (_HG.LEVEL_MULT_PER_LEVEL || 0.08)) * (1 + (stars - 1) * (_HG.STAR_MULT_PER_STAR || 0.15));
    return {
  ...
  get EQUIP_MAX() { return (typeof GAME_BALANCE !== 'undefined' && GAME_BALANCE.EQUIPMENT) ? GAME_BALANCE.EQUIPMENT.MAX_INVENTORY : 500; },
  /**
   * 向装备库存添加一件装备
   * @param {{ uid: string, templateId: string, level: number }} item - 装备实例
   * @returns {object|false} 成功返回装备对象，库存满返回 false
   */
  addEquipment(item) {
    const inv = this.getEquipmentInventory();
    if (inv.length >= this.EQUIP_MAX) return false; // inventory full
    inv.push(item);
    this.saveEquipmentInventory(inv);
    return item;
  },
  ...
  // === War Director Meta Progression ===
  getWarState() {
    return this._get('warState', {
      version: 1,
      points: 0,
      streak: 0,
      doctrinePoints: 0,
  ...
  saveWarState(s) { this._set('warState', s); },
};

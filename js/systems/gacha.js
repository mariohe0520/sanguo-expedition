...
const Gacha = {
  ...
  // Feature 5: Pity threshold — guaranteed success after this many failed attempts
  PITY_THRESHOLD: 5,

  /**
   * 开始拜访一位武将（三顾茅庐系统）
   * @param {string} heroId - 要拜访的武将 ID
   * @returns {{ hero: object, visit: object, dialogue: object, sincerity: number }|null} 拜访状态或 null
   */
  startVisit(heroId) {
    const visit = this.VISITS[heroId];
    if (!visit) return null;

    // Feature 5: Check if hero is already owned — show shards message instead
    const roster = Storage.getRoster();
    const alreadyOwned = !!roster[heroId];

    // Feature 5: Check if hero is blocked by Destiny choices
    if (typeof Destiny !== 'undefined' && Destiny.isHeroBlocked(heroId)) {
      return { error: '此人因天命抉择而无法招募' };
    }

    const player = Storage.getPlayer();
    // Apply Destiny gacha discount if available
    let cost = visit.cost;
    if (typeof Destiny !== 'undefined') {
      const destinyState = Destiny.getState();
      if (destinyState && destinyState.gachaDiscount) {
        cost = Math.floor(cost * (1 - destinyState.gachaDiscount));
      }
    }
    if (player.gold < cost) return { error: '金币不足 (需要' + cost + ')' };
    player.gold -= cost;
    Storage.savePlayer(player);

    const state = Storage.getGachaState();
    if (!state.visits[heroId]) state.visits[heroId] = { sincerity: 0, dialogueIdx: 0, attempts: 0, failedVisits: 0 };
    // Ensure failedVisits field exists (for saves from before this feature)
    if (typeof state.visits[heroId].failedVisits === 'undefined') state.visits[heroId].failedVisits = 0;
    ...
    return {
      ...
      cost,
      alreadyOwned,
      // Feature 5: Show pity progress
      pityProgress: state.visits[heroId].failedVisits,
      pityThreshold: this.PITY_THRESHOLD,
      pityGuaranteed: state.visits[heroId].failedVisits >= this.PITY_THRESHOLD,
    };
  },
  ...
  makeChoice(heroId, choiceIdx) {
    ...
    if (v.dialogueIdx >= dialogues.length) {
      ...
      // Feature 5: Pity system — guarantee success after PITY_THRESHOLD failed attempts
      if (!recruited && (v.failedVisits || 0) >= this.PITY_THRESHOLD) {
        recruited = true;
        pityTriggered = true;
      }

      if (recruited) {
        // Feature 5: Check if already owned — give shards instead
        const roster = Storage.getRoster();
        if (roster[heroId]) {
          const shardAmount = visit.rarity === 5 ? 10 : visit.rarity === 4 ? 5 : 3;
          Storage.addShards(heroId, shardAmount);
          v._gaveShards = shardAmount;
        } else {
          Storage.addHero(heroId);
        }
        // Reset for future visits
        v.sincerity = 0;
        v.failedVisits = 0; // Reset pity counter on success
      } else {
        // Feature 5: Increment pity counter on failure
        v.failedVisits = (v.failedVisits || 0) + 1;
    ...
    return {
      choice,
      sincerity: v.sincerity,
      recruited,
      visitComplete,
      response: pityTriggered
        ? '（感动）你的诚意...我终于明白了。好，我随你走！'
        : choice.response,
      pityTriggered,
      pityProgress: v.failedVisits || 0,
      pityThreshold: this.PITY_THRESHOLD,
      shardsGiven: v._gaveShards || 0,
    };
  },

  // Get visit status for display (Feature 5: includes pity info)
  getVisitStatus(heroId) {
    const state = Storage.getGachaState();
    const v = state.visits[heroId] || { sincerity: 0, dialogueIdx: 0, attempts: 0, failedVisits: 0 };
    return {
      ...v,
      failedVisits: v.failedVisits || 0,
      pityProgress: v.failedVisits || 0,
      pityThreshold: this.PITY_THRESHOLD,
      pityGuaranteed: (v.failedVisits || 0) >= this.PITY_THRESHOLD,
      isOwned: !!Storage.getRoster()[heroId],
    };
  },
